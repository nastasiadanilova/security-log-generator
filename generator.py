...
def random_ip():
    """random external attacker ip address"""
...
def write(filepath, line):
    ...
    with open(filepath, "a") as f:         # "a" = append, add to end of file
        f.write(line + "\n")
    print(line)
...
def ssh_brute_force():
    ...
    if random.random() < 0.02:             # 2% successful break-in
        line = (f"{ts()} {host} sshd[{pid}]: "
                f"Accepted password for {user} from {ip} port {port} ssh2")
    elif random.random() < 0.85:           # wrong password
        line = (f"{ts()} {host} sshd[{pid}]: "
...
def sql_injection():
    ...
    status    = random.choice([400, 403, 500])   # error codes
...
def ddos():
    ...
    # ddos = many requests in a row from one ip
    for _ in range(random.randint(20, 50)):    # burst of requests
        line = (f'{ip} - - [{datetime.now().strftime("%d/%b/%Y:%H:%M:%S +0000")}] '
                f'"GET / HTTP/1.1" 429 {size}')  # 429 = too many requests
        write("logs/nginx/access.log", line)
...
def credential_stuffing():
    ...
    # many different users from same ip — credential stuffing signature
...
def lateral_movement():
    ...
    host  = "ubuntu-server"
    pid   = random.randint(10000, 99999)
    # internal ips — attacker already inside network
...
def normal_traffic():
    ...
    status = random.choice([200, 200, 200, 200, 301, 304])  # mostly 200
...
def normal_ssh():
    ...
    user  = random.choice(users)
    ip    = f"192.168.1.{random.randint(1, 50)}"   # internal network ip
    port  = 22
    pid   = random.randint(10000, 99999)
    host  = "ubuntu-server"

    variants = [
        f"{ts()} {host} sshd[{pid}]: Accepted publickey for {user} from {ip} port {port} ssh2",
...
ATTACKS = {
    ...
    "16": ("all attacks mixed",     None),
    "17": ("mixed with normal",     None),
}
...
def menu():
    ...
    choice = input("  choose attack (1-17): ").strip()
...
def run_interactive():
    ...
    choice = menu()
    _current_mode = choice             # current mode
...
def run_env():
    ...
    _current_mode = key                # save current mode
...
def _loop(fn, interval):
    ...
    try:
        while True:
            if fn is None and _current_mode == "17":
...
if __name__ == "__main__":
    # if ATTACK env variable is set — docker mode, no menu needed
    # if not set — interactive mode with menu
